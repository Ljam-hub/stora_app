...
class UniversalEmailBackend(BaseEmailBackend):
 ...
 sender_name = from_email.split(<)[0].strip().strip('').strip(')
        ...
        try:
            ...
            if not self.fail_silently:
                raise RuntimeError(fBrevo API error {err.code}: {err_body}) from err
            return False
    ...
    def _send_via_resend(self, message, html_content):
        ...
        req = urllib.request.Request(
            ...
            data=json.dumps(payload).encode(utf-8),
            headers={
                Authorization: fBearer {self.resend_api_key},
                Content-Type: application/json,
                Accept: application/json,
                User-Agent: StoraBackend/1.0,
            },
        ...
        try:
            ...
            if not self.fail_silently:
                raise RuntimeError(fResend API error {err.code}: {err_body}) from err
            return False
        except Exception as err:
