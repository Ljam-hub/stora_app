...
@api_view(["GET"])
@permission_classes([AllowAny])
def system_email_status(request):
    import os
    ...
    if not brevo_key:
        return Response({"status": "error", "message": "BREVO_API_KEY is not set"})

    headers = {
        "api-key": brevo_key,
        "Accept": "application/json",
        "User-Agent": "StoraDiagnostic/1.0",
    }

    info = {
        "backend": getattr(django_settings, "EMAIL_BACKEND", None),
        "default_from": getattr(django_settings, "DEFAULT_FROM_EMAIL", None),
    }

    # 1. Brevo Account Info
    try:
        req = urllib.request.Request("https://api.brevo.com/v3/account", headers=headers)
        with urllib.request.urlopen(req, timeout=5) as r:
            info["account"] = json.loads(r.read().decode("utf-8"))
    except Exception as e:
        info["account_error"] = str(e)

    # 2. Brevo Senders
    try:
        req = urllib.request.Request("https://api.brevo.com/v3/senders", headers=headers)
        with urllib.request.urlopen(req, timeout=5) as r:
            info["senders"] = json.loads(r.read().decode("utf-8"))
    except Exception as e:
        info["senders_error"] = str(e)

    # 3. Brevo Transactional Events
    try:
        req = urllib.request.Request("https://api.brevo.com/v3/smtp/statistics/events?limit=10&sort=desc", headers=headers)
        with urllib.request.urlopen(req, timeout=5) as r:
            info["recent_events"] = json.loads(r.read().decode("utf-8"))
    except Exception as e:
        info["events_error"] = str(e)

    # 4. Optional test dispatch if ?to=email
    test_to = request.GET.get("to")
    if test_to:
        from django.core.mail import send_mail
        try:
            res = send_mail(
                "Stora Diagnostic Test",
                "This is a live test email from Stora.",
                getattr(django_settings, "DEFAULT_FROM_EMAIL", "STORA <laisojl014@gmail.com>"),
                [test_to],
                fail_silently=False,
            )
            info["test_send"] = f"SUCCESS (result={res})"
        except Exception as e:
            info["test_send"] = f"FAILED: {e}"
